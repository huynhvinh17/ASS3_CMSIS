 ...
 */
void HAL_GPIO_Init(GPIO_Type *GPIOx, uint32 pin, config_pin_t pinState)
{
    uint32 pinMask = (1 << pin); /** Create a mask to the bit corresponding  */
 ...
 * @param pin pin number on the specified GPIO port
 */
void HAL_GPIO_TogglePin(GPIO_Type *GPIOx, uint32 pin)
{
    uint32 pinMask = (1 << pin); /** Create a mask to the bit corresponding  */
 ...
 */
void HAL_GPIO_WritePin(GPIO_Type *GPIOx, uint32 pin, pin_state_t pinState)
{
